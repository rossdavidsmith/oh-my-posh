...
func TestWTTrackedTime(t *testing.T) {
	...
	for _, tc := range cases {
		...
		env.On("doGet", WTAPIURL).Return([]byte(response), tc.Error)

		cache := &MockedCache{}
		cache.On("get", WTAPIURL).Return(response, !tc.CacheFoundFail)
		cache.On("set", WTAPIURL, response, tc.CacheTimeout).Return()
